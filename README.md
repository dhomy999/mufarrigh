...
### ⚠️ التجريبي لا يُرى في التحديث

نقطة التحديث هي `releases/latest/download/latest.json`، و**GitHub يتخطّى الإصدارات التجريبية (pre-release) عند تحديد «الأحدث»** — فيرجع الرابط 404 ما لم يوجد إصدار مستقرّ واحد على الأقل.

النتيجة عملياً: **أي إصدار تنشره pre-release لن يصل إلى أحد عبر التحديث التلقائي**، وسيبقى فحص التحديث عند المستخدمين صامتاً بلا نتيجة (لا يظهر خطأ — `checkForUpdate` يبتلع فشل الشبكة عمداً كي لا يعطّل الإقلاع).

فإن أردت لإصدارٍ أن يصل إلى المستخدمين، اجعله **إصداراً كاملاً لا تجريبياً**.
